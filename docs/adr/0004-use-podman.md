...
*   **Reliable Edge Deployment (Systemd):** Podman is designed to work closely with systemd. It supports **Quadlet files** (`.container`, `.volume`, `.network` systemd units) for native service management. This is critical for our edge device use case, ensuring all containers automatically and securely restart after a power failure without needing a complex orchestration layer. See **[ADR-0014](0014-dual-deployment-strategy.md)** for the full deployment strategy.
