...
*   **Podman:** Short service names in compose files (`recorder`) keep configs readable and DRY. Explicit container names (`silvasonic-recorder`) ensure they are easily greppable on the host system.

### Detailed Rules

#### 1. Python Packages & Modules
*   **Package Names:** `silvasonic-<service-name>` (e.g. `silvasonic-recorder`)
*   **Import Names:** `silvasonic.<service_name>` (e.g. `from silvasonic.recorder import main`)
*   **Directory Structure:** `src/silvasonic/<service_name>/`

#### 2. Container & Infrastructure
*   **Podman Service:** `<service-name>` (e.g. `recorder`)
*   **Container Name:** `silvasonic-<service-name>` (e.g. `silvasonic-recorder`)
...
*   **Long Service Names in Compose:** Rejected because it makes `compose.yml` verbose and redundant (e.g. `services: silvasonic-recorder:`).
