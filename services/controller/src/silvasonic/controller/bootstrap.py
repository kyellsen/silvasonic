...
class ProfileReader:
    ...
    def read_all(self) -> list[dict[str, Any]]:
        """Read all YAML profile files and return them as a list of dicts."""
        ...
        if not profiles:
            logger.info("no_profiles_found_on_disk")
...
class ProfileBootstrapper:
    ...
    def __init__(self, profiles_dir: str = "/etc/silvasonic/profiles") -> None:
        """Initialize with path to profiles directory."""
        self.reader = ProfileReader(profiles_dir)

    async def sync(self) -> None:
        """Read all YAML profiles and upsert them into the DB."""
        profiles_to_sync = self.reader.read_all()
